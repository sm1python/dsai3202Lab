# this is genetic_algorithms_functions.py:
import numpy as np
...
def calculate_fitness(route,
    ...
    for i in range(len(route) - 1):
        ...
        if distance == 10000:
            return -1e6  # Return POSITIVE penalty for minimization
        total_distance += distance
    return -total_distance  # Return ACTUAL distance to minimize
...
def select_in_tournament(population,
    ...
    population_indices = np.arange(len(population))
    for _ in range(number_tournaments):
        participants = np.random.choice(population_indices, tournament_size, replace=False)
        # Find participant with LOWEST score (since we minimize distance)
        best_idx = participants[np.argmin(scores[participants])]
